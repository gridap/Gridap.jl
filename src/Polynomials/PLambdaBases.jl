...
#                      (I_id,           I, sgnIcomp)
const Component = Tuple{ Int, Vector{Int},      Int}
# where (I_id, I, sgnIcomp) =
#     (            _combination_index(I) , I,                   1 ) for k-forms components,
#  or (_combination_index(_complement(I)), I, _combination_sign(I)) for vector proxy
# for I in _sorted_combinations(D,k)
...
polynomial defined by `ids`, and cardinal of the parent basis.

It is 0 if the basis is empty.
"""
...
function Base.show(io::IO, ::MIME"text/plain", indices::PLambdaIndices)
  ...
  k = length(I)
  D = length(α)-length(J)+length(I)-1
...
"""
    PmLambdaBasis{D,V,L,B} <: PolynomialBasis{D,V,Bernstein}
...
The number of basis polynomials is binomial(`r`+`k`-1,`k`)*binomial(`D`+`r`,`D`-`k`) if no custom bubble indices are given.
...
"""
    PmLambdaBasis(::Val{D}, T, r, k, vertices=nothing)
...
"""
    PLambdaBasis(::Val{D}, T, r, k, vertices=nothing)
...
Given a k-combination `combi` of elements of 1:`D`, returns the unique `D`-k
combination of 1:D `comp` such that `combi` ∪ `comp` == 1:`D`.
"""
...
"""
    _combination_sign(combi)

Given a combination `combi`, returns the sign of the permutation resulting from
the concatenation of `combi` and its complement [`_complement(combi)`](@ref _complement).
"""
...
Return the set of indices `i` in the given bernstein multi-index `α` such that `αᵢ`>0.
"""
function _support(α)
  s = Int[]
