...
"""
struct FaceLabels
  dim_to_nface_to_geolabel::Vector{IndexCellValue}
  phystag_to_geolabels::Vector{Vector{Int}}
  name_to_tag::Dict{String,Int}
end

function FaceLabels(
  dim_to_nface_to_geolabel::Vector{Vector{Int}},
  physlabel_to_geolabels::Vector{Vector{Int}},
  name_to_tag::Dict{String,Int})

  cv = [ CellValueFromArray(v) for v in dim_to_nface_to_geolabel ]
  FaceLabels(cv, physlabel_to_geolabels,name_to_tag)
end
...
"""
labels_on_dim(l::FaceLabels,dim::Integer) = l.dim_to_nface_to_geolabel[dim+1]

"""
Returns a Vector{Int} with the goelabels associated with a given phystag
"""
labels_on_tag(l::FaceLabels,tag::Integer) = l.phystag_to_geolabels[tag]

ntags(l::FaceLabels) = length(l.phystag_to_geolabels)

tag_from_name(l::FaceLabels,name::String) = l.name_to_tag[name]
