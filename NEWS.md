...
- Implemented moment-based scalar (`H1` conform) elements for FEEC families `F=:P⁻`/`:P`/`:Q⁻`/`:S` under the name `ModalScalar{F}`. They are the default elements when calling for `k`=0-forms in the generic FEEC reference FE constructor, use the keyword `nodal=true` to opt-in nodal DOF based counterpart (`lagrangian` or `serendipity`). Since PR[#1173](https://github.com/gridap/Gridap.jl/pull/1173).
