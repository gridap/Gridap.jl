...
al = lazy_map(BlockArrayCooMap(blockids),ax,b11,b21,b32)
...
bl = lazy_map(BlockArrayCooMap(blockids),ax,b1,b2)

b2 = [  i*[3,4,5,6] for i in 1:l]
blockids = [(2,)]
ax = Fill((blockedrange([2,4]),),l)
bl = lazy_map(BlockArrayCooMap(blockids),ax,b2)

c1 = [  i*[7,8] for i in 1:l]
c2 = [  i*[3,4,5] for i in 1:l]
blockids = [(1,),(2,)]
ax = Fill((blockedrange([2,3,2]),),l)
cl = lazy_map(BlockArrayCooMap(blockids),ax,c1,c2)
...
al = lazy_map(BlockArrayCooMap(blockids),ax,a11,a12)
...
ax = CompressedArray([ax1,ax2],vcat(fill(1,l1),fill(2,l2)))
aBl = lazy_map(BlockArrayCooMap(blockids),ax,al)
...
ax = CompressedArray([ax1,ax2],vcat(fill(1,l1),fill(2,l2)))
bBl = lazy_map(BlockArrayCooMap(blockids),ax,bl)
