module ExteriorFormValuesTests
# Construction and pointwise algebra of ExteriorFormValue:
...
ExteriorFormValue{0,2}((1,))
ExteriorFormValue{1,2}((1,2))
ExteriorFormValue{2,2}((1,))

ExteriorFormValue{0,3}((1,))
ExteriorFormValue{1,3}((1,2,3))
ExteriorFormValue{2,3}((1,2,3))
ExteriorFormValue{3,3}((1,))

ExteriorFormValue{0,4}((1,))
ExteriorFormValue{1,4}((1,2,3,4))
ExteriorFormValue{2,4}((1,2,3,4,5,6))
ExteriorFormValue{3,4}((1,2,3,4))
ExteriorFormValue{4,4}((1,))

# ── Constructor family ───────────────────────────────────────────────────────

# Homogeneous NTuple, with and without an explicit component type
@test isa(ExteriorFormValue{1,3}((1,2,3)),            ExteriorFormValue{1,3,Int,3})
@test isa(ExteriorFormValue{1,3,Float64}((1,2,3)),    ExteriorFormValue{1,3,Float64,3})
@test isa(ExteriorFormValue{1,3,Float64,3}((1,2,3)),  ExteriorFormValue{1,3,Float64,3})
@test isa(ExteriorFormValue{1,3,Float64,3}((1.,2.,3.)), ExteriorFormValue{1,3,Float64,3})

# Heterogeneous Tuple: promoted, or converted to the requested type
@test isa(ExteriorFormValue{1,3}((1,2.0,3)),          ExteriorFormValue{1,3,Float64,3})
@test isa(ExteriorFormValue{1,3,Int}((1,2.0,3)),      ExteriorFormValue{1,3,Int,3})
@test isa(ExteriorFormValue{1,3,Int,3}((1,2.0,3)),    ExteriorFormValue{1,3,Int,3})

# Vararg
@test ExteriorFormValue{1,3}(1,2.0,3)       == ExteriorFormValue{1,3}((1.0,2.0,3.0))
@test ExteriorFormValue{1,3,Int}(1,2.0,3)   == ExteriorFormValue{1,3}((1,2,3))
@test ExteriorFormValue{1,3,Int,3}(1,2.0,3) == ExteriorFormValue{1,3}((1,2,3))

# A single scalar is a full 0-form and a full D-form
@test isa(ExteriorFormValue{0,3}(1),   ExteriorFormValue{0,3,Int,1})
@test isa(ExteriorFormValue{0,0}(1),   ExteriorFormValue{0,0,Int,1})
@test isa(ExteriorFormValue{3,3}(1.0), ExteriorFormValue{3,3,Float64,1})

# K > D: the space is trivial, so the value has no component
@test isa(ExteriorFormValue{4,3}(),          ExteriorFormValue{4,3,Int,0})
@test isa(ExteriorFormValue{4,3}(()),        ExteriorFormValue{4,3,Int,0})
@test isa(ExteriorFormValue{4,3,Float64}(),  ExteriorFormValue{4,3,Float64,0})
@test isa(ExteriorFormValue{4,3,Float64}(()),ExteriorFormValue{4,3,Float64,0})
@test isa(ExteriorFormValue{4,3,Int,0}(),    ExteriorFormValue{4,3,Int,0})
@test isa(ExteriorFormValue{4,3,Int,0}(()),  ExteriorFormValue{4,3,Int,0})

@test_throws AssertionError ExteriorFormValue{1,3}((1,2))
@test_throws AssertionError ExteriorFormValue{1,3,Int}((1,2))
@test_throws AssertionError ExteriorFormValue{1,3,Int,2}((1,2))
@test_throws AssertionError ExteriorFormValue{1,3,Int,2}(1,2)
@test_throws AssertionError ExteriorFormValue{0,3}()
@test_throws AssertionError ExteriorFormValue{0,3,Int}()

# ── Independent component count ──────────────────────────────────────────────

@test num_indep_components(ExteriorFormValue{0,2}) == 1
@test num_indep_components(ExteriorFormValue{2,2}) == 1
@test num_indep_components(ExteriorFormValue{1,3}) == 3
@test num_indep_components(ExteriorFormValue{2,3}) == 3
@test num_indep_components(ExteriorFormValue{4,3}) == 0
...
A = convert(SArray{Tuple{3,3},Int}, ExteriorFormValue{2,3}((1,2,3)))
@test A == [0 1 2; -1 0 3; -2 -3 0]
@test A == -transpose(A)
@test isa(convert(MArray{Tuple{3,3},Int}, ExteriorFormValue{2,3}((1,2,3))), MMatrix{3,3,Int,9})

@test eltype(convert(SArray{Tuple{3,3},Float64}, ExteriorFormValue{2,3}((1,2,3)))) == Float64

# A 1-form gives back its components, a 0-form a zero-dimensional array
@test convert(SArray{Tuple{3},Int}, ExteriorFormValue{1,3}((1,2,3))) == [1,2,3]
@test convert(SArray{Tuple{},Int}, ExteriorFormValue{0,3}((7,)))[] == 7

# The volume form is the Levi-Civita symbol scaled by its single component
A3 = convert(SArray{Tuple{3,3,3},Int}, ExteriorFormValue{3,3}((2,)))
@test (A3[1,2,3], A3[2,1,3], A3[3,1,2], A3[1,1,3]) == (2, -2, 2, 0)

# K > D: no stored component, but the tensor still has D^K (vanishing) entries
A4 = convert(SArray{Tuple{3,3,3,3},Int}, ExteriorFormValue{4,3}())
@test size(A4) == (3,3,3,3)
@test all(iszero, A4)

# The wedge of two 1-forms expands to the antisymmetric part of their outer product
a1 = ExteriorFormValue{1,4}((1,2,3,4))
b1 = ExteriorFormValue{1,4}((5,6,7,8))
...
@test_throws DimensionMismatch convert(SArray{Tuple{2,2},Int}, ExteriorFormValue{2,3}((1,2,3)))

# Internal conversion changes the component type, and is a no-op on its own type
@test convert(ExteriorFormValue{2,3,Float64}, ExteriorFormValue{2,3}((1,2,3))) ==
      ExteriorFormValue{2,3}((1.0,2.0,3.0))
ω23 = ExteriorFormValue{2,3}((1.0,2.0,3.0))
@test convert(ExteriorFormValue{2,3,Float64}, ω23) === ω23

# ── zero, +, -, * ────────────────────────────────────────────────────────────

a3 = ExteriorFormValue{1,3}((1.0, 2.0, 3.0))
b3 = ExteriorFormValue{1,3}((4.0, 5.0, 6.0))
...
D = 2
a = ExteriorFormValue{0,D}((4,))
b = ExteriorFormValue{1,D}((1,1))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{1,D}
@test c.data == (4.0,4.0)

# ∧ of 1-forms in 2D
a = ExteriorFormValue{1,D}((0,1))
b = ExteriorFormValue{1,D}((1,0))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{2,D}
@test c.data == (-1.0,)

a = ExteriorFormValue{1,D}((1,0))
b = ExteriorFormValue{1,D}((0,1))
@test (a ∧ b).data == (1.0,)
...
a = ExteriorFormValue{1,D}((1,1))
b = ExteriorFormValue{1,D}((1,1))
@test (a ∧ b).data == (0.0,)

a = ExteriorFormValue{1,D}((4,1))
b = ExteriorFormValue{1,D}((1,1))
@test (a ∧ b).data == (3.0,)

# ∧ of 1-form and 2-form in 2D: K1+K2 = 3 > D = 2, result is the zero 3-form
a = ExteriorFormValue{1,D}((4,1))
b = ExteriorFormValue{2,D}((1,))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{3,D}
@test c.data == ()

# ∧ of 0-form and 2-form in 2D
a = ExteriorFormValue{0,D}((4.0,))
b = ExteriorFormValue{2,D}((1.0,))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{2,D}
@test c.data == (4.0,)
...
# 3D wedges
D = 3
a = ExteriorFormValue{1,D}((0,0,1))
b = ExteriorFormValue{1,D}((1,0,0))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{2,D}
@test c.data == (0.0,-1.0,0.0)

a = ExteriorFormValue{1,D}((1,0,0))
b = ExteriorFormValue{1,D}((1,1,0))
@test (a ∧ b).data == (1.0,0.0,0.0)

a = ExteriorFormValue{1,D}((1,0,0))
b = ExteriorFormValue{2,D}((1,1,0))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{3,D}
@test c.data == (0.0,)

a = ExteriorFormValue{1,D}((0,0,1))
b = ExteriorFormValue{2,D}((1,0,0))
@test (a ∧ b).data == (1.0,)

a = ExteriorFormValue{0,D}((4.0,))
b = ExteriorFormValue{2,D}((1.0,0.0,0.0))
c = a ∧ b
@test typeof(c) <: ExteriorFormValue{2,D}
...
dx1_2d = ExteriorFormValue{1,D2}((1.0, 0.0))
dx2_2d = ExteriorFormValue{1,D2}((0.0, 1.0))

# ι_eᵢ(dxʲ) = δᵢʲ  (0-form = scalar)
r11 = ι(e1, dx1_2d);  @test typeof(r11) <: ExteriorFormValue{0,D2};  @test r11.data == (1.0,)
...
vol2 = ExteriorFormValue{2,D2}((1.0,))
rv1  = ι(e1, vol2);  @test typeof(rv1) <: ExteriorFormValue{1,D2};  @test rv1.data == (0.0, 1.0)
...
vol3 = ExteriorFormValue{3,D3}((1.0,))
r3   = ι(e13, vol3)
@test typeof(r3) <: ExteriorFormValue{2,D3}
@test r3.data == (0.0, 0.0, 1.0)   # dx²∧dx³ is the third 2-combination

# ── Hodge star (flat Euclidean) ──────────────────────────────────────────────

# 3D: ⋆dx¹ = dx²∧dx³,  ⋆dx² = -dx¹∧dx³,  ⋆dx³ = dx¹∧dx²
dx1_3d = ExteriorFormValue{1,D3}((1.0, 0.0, 0.0))
dx2_3d = ExteriorFormValue{1,D3}((0.0, 1.0, 0.0))
dx3_3d = ExteriorFormValue{1,D3}((0.0, 0.0, 1.0))
...
# 0-form → 3-form
f0 = ExteriorFormValue{0,D3}((5.0,))
sf0 = hodge_star(f0)
@test typeof(sf0) <: ExteriorFormValue{3,D3}
@test sf0.data == (5.0,)

# 3-form → 0-form
svol3 = hodge_star(vol3)
@test typeof(svol3) <: ExteriorFormValue{0,D3}
@test svol3.data == (1.0,)
...
@test typeof(sdx1_2d) <: ExteriorFormValue{1,D2}
...
_full(ω::ExteriorFormValue) = MultiValue(SArray(ω))
...
for (α, β) in [(ExteriorFormValue{0,D3}((7.0,)),
                ExteriorFormValue{0,D3}((3.0,))),
               (ExteriorFormValue{1,D3}((1.0, 2.0, 3.0)),
                ExteriorFormValue{1,D3}((4.0, 5.0, 6.0))),
               (ExteriorFormValue{2,D3}((1.0, 2.0, 3.0)),
                ExteriorFormValue{2,D3}((4.0, 5.0, 6.0))),
               (ExteriorFormValue{3,D3}((2.0,)),
                ExteriorFormValue{3,D3}((5.0,)))]
  K = length(size(α))
...
ω_show = ExteriorFormValue{1,D3}((1.0, 2.0, 3.0))
...
a_i  = ExteriorFormValue{1,D3,Int}((1,2,3))
b_f  = ExteriorFormValue{1,D3,Float32}((1f0,2f0,3f0))
...
triv = ExteriorFormValue{4,D3,Float32}(())

# ι lands in the nontrivial Λ³: a zero form, not an empty one
@test ι(v_i, triv) === zero(ExteriorFormValue{3,D3,Float32})
@test a_i ∧ triv   === zero(ExteriorFormValue{5,D3,Float32})
@test pullback(triv, Jt_f) === zero(ExteriorFormValue{4,2,Float32})
...
@test apply_form(ExteriorFormValue{4,D3,Int}(()), v_i, v_i, v_i, v_i) === zero(Int)
@test apply_form(ExteriorFormValue{1,0,Float32}(()), VectorValue{0,Float32}()) === zero(Float32)
@test form_inner(triv, ExteriorFormValue{4,D3,Int}(())) === zero(Float32)
@test form_inner(triv, ExteriorFormValue{4,D3,Int}(()), g_f) === zero(Float32)
# the conversions name the eltype, so it survives an empty container
@test to_Kform(VectorValue{0,Float32}(()), Val(4), Val(D3)) ===
      zero(ExteriorFormValue{4,D3,Float32})
@test from_Kform(triv) === zero(VectorValue{0,Float32})
@test to_1form(VectorValue{0,Float32}()) === zero(ExteriorFormValue{1,0,Float32})
@test from_1form(ExteriorFormValue{1,0,Float32}(())) === zero(VectorValue{0,Float32})
