...
# dof_basis = DivConformingDOFBasis(int_points, moments)
#
# divreffe = DivConformingRefFE(p,dof_basis,basis,mynfacedofs)
...
# struct DivConformingDOFBasis{D,T,S} <: DOFBasis{D,T}
...
#   polytope::Polytope{D}
#   dof_basis::DivConformingDOFBasis{D,T}
#   shfbasis::Basis{D,T}
#   nfacedofs::Vector{Vector{Int}}
# end
#
# function DivConformingRefFE(p::Polytope{D}, dof_basis::DivConformingDOFBasis{D,T,S}, shfbasis::Basis{D,S}, nfacedofs) where {D,T,S}
#   DivConformingRefFE{D,S}(p,dof_basis,shfbasis,nfacedofs)
# end
#
# # function DivConformingDOFBasis(nodes::Vector{Array{Point{D,T}}}, moments::Vector{Array{Point{D,T}}}) where {D,T}
# function DivConformingDOFBasis(nodes::Vector{Array{Point{D,S}}}, moments::Vector{Array{T}}) where {D,T,S}
#   ndofs = sum(size(moments[i],1) for i in 1:length(moments))
#   cache_field = zeros(S,ndofs)
#   cache_basis = zeros(S,ndofs,ndofs) # TO CHECK !!!
#
#   DivConformingDOFBasis{D,T,S}(
#     nodes,
#     moments,
#     cache_field,
#     cache_basis)
