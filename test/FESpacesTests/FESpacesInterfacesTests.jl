...
@test isa(dv.minus(x_Γ)[1],GBlock)
@test isa(du.plus(x_Γ)[1],GBlock)
@test isa(∇(dv).plus(x_Γ)[1],GBlock)
@test isa(∇(du).minus(x_Γ)[1],GBlock)
...
cellidsR = [2,4,3,1]
cellidsS = SkeletonPair(cellidsL,cellidsR)
@test isa(get_cell_dof_ids(V,cellidsS)[1],GBlock)
