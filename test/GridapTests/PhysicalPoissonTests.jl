...
for data in [ vector_data, scalar_data ]
  ...
  V = TestFESpace(
   ...
   dof_space = :physical)
   # dirichlet_tags="dirichlet")
  # Juno.@enter TestFESpace( model=model, order=order, reffe=:Lagrangian, labels=labels, valuetype=T, dirichlet_tags="dirichlet", dof_space = :physical)
