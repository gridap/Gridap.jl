...
ϕl = GenericCellMap(lincomb(fl,cl))
...
zf = convert_to_cell_field(zl,length(ϕl))∘inverse_map(ϕl)
df = af*zf
...
@test isa(get_array(get_coordinates(quad)),AppendedArray)
