...
ag = lazy_map_to_field_array(FieldOpKernel(+),af,af)
gx = fill(v+v,np)
...
ag = lazy_map_to_field_array(FieldPlaceHolder,FieldOpKernel(+),af,af)
test_array(evaluate_field_array(ag,ax),agx)

c = 0.5
ac = fill(c,l)
ag = lazy_map_to_field_array(FieldOpKernel(*),ac,af)
gx = fill(c*v,np)
...
w = 2.0
aw = fill(w,l)
ag = lazy_map_to_field_array(FieldOpKernel(+),af,aw)
gx = fill(v+w,np)
...
ag = lazy_map_to_field_array(FieldPlaceHolder,FieldOpKernel(+),af,aw)
test_array(evaluate_field_array(ag,ax),agx)
