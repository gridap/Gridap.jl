...
> In the simplest case, the time step $h = h_{n} = t_{n+1} - t_{n}$ prescribed and constant across all iterations. The state vectors are simply the initial conditions, i.e. $s = n$ and $\mathcal{I} = \mathrm{id}$, and assuming that the initial conditions are given by increasing order of time derivative, $\mathcal{F}$ returns its first input.
>
...
## $\theta$-method
This scheme is used to solve first-order ODEs and relies on the simple state vector $\\{\boldsymbol{s}(t)\\} = \\{\boldsymbol{u}(t)\\}$. This means that the starting and finishing procedure are simply the identity.
...
This scheme was originally devised to control the damping of high frequencies. One parameteristation consists in prescribing the eigen values at $|z| \to +\infty$, and this leads to
...
It is easier to consider the limit cases $|z| \to 0$ and $|z| \to +\infty$ and look at the eigen values of the amplification matrix.
