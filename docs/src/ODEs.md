...
\newcommand{\cci}[2]{\llbracket#1, #2\rrbracket}
...
We consider an initial value problem written in the form $$\left\{\begin{array}{rcll}\mb{r}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) &=& \mb{0}_{d}, \\ \partial_{t}^{k} \mb{u}(t_{0}) &=& \mb{u}_{0}^{k} & \forall k \in \cci{0}{n-1},\end{array}\right.$$ where
* $\mb{u}: \RR \to \RR^{d}$ is the unknown of the problem,
* $n \in \NN$ is the order of the ODE,
* $t_{0} \in \RR$ is the initial time and $\{\mb{u}_{0}^{k}\}_{0 \leq k \leq n-1} \in (\RR^{d})^{n-1}$ are the initial conditions, and
* $\mb{r}: \RR \times (\RR^{d})^{n} \to \RR^{d}$ is the residual.

> We illustrate these notations on the semi-discretisation of the heat equation. It is a first-order ODE so we have $n = 1$, and $d$ is the number of degrees of freedom. The residual and initial condition have the form $$\mb{r}(t, \mb{u}, \dot{\mb{u}}) \doteq \mb{M} \dot{\mb{u}} + \mb{K}(t) \mb{u} - \mb{f}(t), \qquad \mb{u}(t_{0}),$$ where $\mb{M} \in \RR^{d \times d}$ is the mass matrix, $\mb{K}: \RR \to \RR^{d \times d}$ is the stiffness matrix, and $\mb{f}: \RR \to \RR^{d}$ is the forcing term.

Suppose that we are willing to approximate $\mb{u}$ at a time $t_{F} > t_{0}$. A numerical scheme splits the time interval $[t_{0}, t_{F}]$ into smaller intervals $[t_{n}, t_{n+1}]$ (that do not have to be of equal length) and propagates the information at time $t_{n}$ to time $t_{n+1}$. More formally, we consider a general framework consisting of a starting, an update and a finishing map defined as follows
* The **starting map** $\mc{I}: (\RR^{d})^{n} \to (\RR^{d})^{s}$ converts the initial conditions into $s$ state vectors, where $s \geq n$.
* The **marching map** $\mc{U}: \RR \times (\RR^{d})^{s} \to (\RR^{d})^{s}$ updates the state vectors from time $t_{n}$ to time $t_{n+1}$.
* The **finishing map** $\mc{F}: \RR \times (\RR^{d})^{s} \to \RR^{d}$ converts the state vectors into the evaluation of $\mb{u}$ at the current time.

> In the simplest case, the time step $h = h_{n} = t_{n+1} - t_{n}$ prescribed and constant across all iterations. The state vectors are simply the initial conditions, i.e. $s = n$ and $\mc{I} = \mathrm{id}$, and assuming that the initial conditions are given by increasing order of time derivative, $\mc{F}$ returns its first input.
>
> Some schemes need nontrivial starting and finishing maps. (See the generalised-$\alpha$ schemes below.) When higher-order derivatives can be retrieved from the state vectors, it is also possible to take another definition for $\mc{F}$ so that it returns the evaluation of $\mb{u}$ and higher-order derivatives at the current time.

These three maps need to be designed such that the following recurrence produces approximations of $\mb{u}$ at the times of interest $t_{n}$
$$\left\{\begin{array}{rcl}
\{\mb{s}\}_{n+1} &=& \mc{U}(h_{n}, \{\mb{s}\}_{n}) \\
\{\mb{s}\}_{0} &=& \mc{I}(\mb{u}_{0}^{0}, \ldots, \mb{u}_{0}^{n-1})
\end{array}\right., \qquad \mb{u}_{n} = \mc{F}(\{\mb{s}\}_{n}).$$
More precisely, we would like $\mb{u}_{n}$ to be close to $\mb{u}(t_{n})$. Here the notation $\{\mb{s}\}_{n}$ stands for the state vector, i.e. a vector of $s$ vectors: $\{\mb{s}\}_{n} = (\mb{s}_{n, i})_{1 \leq i \leq s}$. In particular, we notice that we need the exactness condition $\mc{F} \circ \mc{I}(\mb{u}_{0}^{0}, \ldots, \mb{u}_{0}^{n-1}) = \mb{u}_{0}$. This is a condition on the design of the pair ($\mc{I}$, $\mc{F}$).

# Classification of ODEs and numerical schemes
Essentially, a numerical scheme converts a (continuous) ODE into (discrete) nonlinear systems of equations. These systems of equations can be linear under special conditions on the nature of the ODE and of the numerical scheme. Since numerical methods for linear and nonlinear systems of equations can be quite different in terms of cost and implementation, we are interested in solving linear systems whenever possible. This leads us to perform the following classifications.
...
* **Quasilinear**. The residual is linear with respect to the highest-order time derivative, and the corresponding linear form may depend on time and lower-order time derivatives, i.e.$$\mb{r}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) = \mb{M}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n-1} \mb{u}) \partial_{t}^{n} \mb{u} + \mb{f}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n-1} \mb{u}).$$ We call the matrix $\mb{M}: \RR \to \RR^{d \times d}$ the mass matrix. In particular, a quasilinear ODE is a nonlinear ODE.
* **Semilinear**. The residual is quasilinear and the mass matrix may only depend on time, i.e.$$\mb{r}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) = \mb{M}(t) \partial_{t}^{n} \mb{u} + \mb{f}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n-1} \mb{u}).$$ In particular, a semilinear ODE is a quasilinear ODE.
* **Linear**. The residual is linear with respect to all time derivatives, i.e.$$\mb{r}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) = \sum_{0 \leq k \leq n} \mb{A}_{k}(t) \partial_{t}^{k} \mb{u} + \mb{f}(t).$$ We refer to the matrix $\mb{A}_{k}: \RR \to \RR^{d \times d}$ as the $k$-th linear form of the residual. We may still define the mass matrix $\mb{M} = \mb{A}_{n}$. In particular, a linear ODE is a semilinear ODE.
...
We consider an extra ODE type that is motivated by stiff problems. We say that an ODE has an implicit-explicit (IMEX) decomposition if it be can written as the sum of a residual of order $n$ and another residual of order $n-1$, i.e. $$\mb{r}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) = \mb{r}_{\text{implicit}}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n} \mb{u}) + \mb{r}_{\text{explicit}}(t, \partial_{t}^{0} \mb{u}, \ldots, \partial_{t}^{n-1} \mb{u}).$$ The decomposition takes the form above so that the mass matrix of the global residual is fully-contained in the implicit part. The table below indicates the type of the corresponding global ODE.
...
Generally speaking, we split the time domain $[t_{0}, t_{F}]$ into

We introduce a classification of numerical schemes based on where they evaluate the residual during the state update.

* If it is possible (up to a change of variables) to write the system of equation for the state update as evaluations of the residual at known values (that depend on the solution at the current time) for all but the highest-order derivative, we say that the scheme is explicit.
* Otherwise, we say that the scheme is implicit.

> For example, when solving a first-order ODE, the state update would involve solving one or more equations of the type $$\mb{r}(t_{k}, \mb{u}_{k}(\mb{x}), \mb{v}_{k}(\mb{x})) = \mb{0},$$ where $\mb{x}$ an the unknown of the state update. The scheme is explicit if it is possible to introduce a change of variables such that $\mb{u}_{k}$ does not depend on $\mb{x}$. Otherwise it is implicit.
...
The time-dependent analogue of `FEOperator` is `TransientFEOperator`. It has the following constructors based on the nonlinearity type of the underlying ODE.
...
It is important to note that he residual assumes that all terms are brought to the same side. In particular, in the common case where the forcing term is on the right hand side, it will need to be made negative in this description.
...
## Stage operator
A `StageOperator` represents the linear or nonlinear operator that a numerical scheme relies on to evolve the state vector. It is essentially a special kind of `NonlinearOperator` but it overwrites the behaviour of nonlinear and linear solvers, in order to take advantage of the matrix of a linear system being constant. The two following subtypes of `StageOperator` are the building blocks of all numerical schemes. 
* `LinearStageOperator` represents the system $\mb{J} \mb{x} + \mb{r} = \mb{0}$, and can build $\mb{J}$ and $\mb{r}$ by evaluating the residual at a given point.
* `NonlinearStageOperator` represents $\mb{r}(\mb{t}, \mb{\ell}_{0}(\mb{x}), \ldots, \mb{\ell}_{N}(\mb{x})) = \mb{0}$, where it is assumed that all the $\mb{\ell}_{k}(\mb{x})$ are linear in $\mb{x}$.

## ODE solution
This type is a simple wrapper around an `ODEOperator`, an `ODESolver` and initial conditions that can be iterated over to evolve the ODE.

# Numerical schemes formulation and implementation
We conclude this note by some description of numerical schemes and their implementation in Gridap.

Suppose that the scheme has been evolved up to time $t_{n}$ already and that the state vectors $\{\mb{s}\}_{n}$ are known. We are willing to evolve the ODE up to time $t_{n+1} > t_{n}$, i.e. compute the state vectors $\{\mb{s}\}_{n+1}$. Generally speaking, a numerical scheme constructs an approximation of the map $\{\mb{s}\}_{n} \to \{\mb{s}\}_{n+1}$ by solving one or more relationships of the type
$$\mb{r}(t_{i}, \Delta_{i}^{0}(\{\mb{s}\}_{n}, \{\mb{s}\}_{n+1}), \ldots, \Delta_{i}^{n}(\{\mb{s}\}_{n}, \{\mb{s}\}_{n+1})) = \mb{0},$$ 
where $t_{i}$ is an intermediate time and $\Delta_{i}^{k}$ are discrete operators that approximates the $k$-th order time derivative of $\mb{u}$ at time $t_{i}$.

We now describe the numerical schemes implemented in Gridap using this framework. It is usually convenient to perform a change of variables so that the unknown $\mb{x}$ has the dimension of the highest-order time derivative of $\mb{u}$, i.e. $[\mb{x}] = [t]^{-n} [\mb{u}]$ (where $[\bullet]$ stands for "the dimension of $\bullet$"). We always perform such a change of variable in practice.

We also briefly characterise these schemes in terms of their order and linear stability.

## $\theta$-method
This scheme is used to solve first-order ODEs and relies on the simple state vector $\{\mb{s}(t)\} = \{\mb{u}(t)\}$. This means that the starting and finishing procedure are simply the identity.

The $\theta$-method relies on the following approximation
$$\mb{u}(t_{n+1}) = \mb{u}(t_{n}) + \int_{t_{n}}^{t_{n+1}} \partial_{t} \mb{u}(t) \D{t} \approx \mb{u}(t_{n}) + h_{n} \partial_{t} \mb{u}(t_{n + \theta}),$$
where we have introduced the intermediate time $t_{n + \theta} \doteq (1 - \theta) t_{n} + \theta t_{n+1}$. By replacing $\mb{u}(t_{n})$ and $\mb{u}(t_{n+1})$ by their discrete equivalents, we have $\partial_{t} \mb{u}(t_{n + \theta}) \approx \frac{1}{h} (\mb{u}_{n+1} - \mb{u}_{n})$. This quantity is found by enforcing that the residual is zero at $t_{n + \theta}$. In that sense the $\theta$-method can be framed as a collocation method at $t_{n + \theta}$. For that purpose, we use the same quadrature rule as above to approximate $\mb{u}(t_{n + \theta})$, i.e. $\mb{u}(t_{n + \theta}) \approx \mb{u}_{n} + \theta h_{n} \partial_{t} \mb{u}(t_{n + \theta})$. Using the notations of the framework above, we have defined
$$\begin{align*}
t_{1} &= (1 - \theta) t_{n} + \theta t_{n+1}, \\
\Delta_{1}^{0} &= (1 - \theta) \mb{u}_{n} + \theta \mb{u}_{n+1}, \\
\Delta_{1}^{1} &= \frac{1}{h} (\mb{u}_{n+1} - \mb{u}_{n}).
\end{align*}$$

To summarize and to be more concrete, let $\mb{x} = \frac{1}{h} (\mb{u}_{n+1} - \mb{u}_{n})$. The $\theta$-method solves the following stage operator
$$\mb{r}(t_{n} + \theta h_{n}, \mb{u}_{n} + \theta h_{n} \mb{x}, \mb{x}) = \mb{0},$$
and sets $\mb{u}_{n+1} = \mb{u}_{n} + h_{n} \mb{x}$. The output state is simply $\{\mb{s}\}_{n+1} = \{\mb{u}_{n+1}\}$. 

**Analysis**
Since this scheme uses $\mb{u}(t)$ as its only state vector, the amplification matrix has dimension one and its coefficient is the stabilisation function, given by
$$\rho(z) = \frac{1 + (1 - \theta) z}{1 - \theta z}.$$
We plug the Taylor expansion of $\mb{u}_{n+1}$ around $\mb{u}_{n}$ in $\mb{u}_{n+1} = \rho(z) \mb{u}_{n}$ and obtain the exactness condition $\rho(z) - \exp(z) = 0$. We then seek to match as many coefficients in the Taylor expansion of both sides to obtain order conditions. We readily obtain the following expansion
$$\rho(z) - \exp(z) = \sum_{k \geq 0} \left[\theta^{k} - \frac{1}{(k+1)!}\right] z^{k+1}.$$
The order conditions are as follows.
* **Order 0 and 1**. The firs two coefficients are always zero, so the method has at least order one.
...
This scheme relies on the state vector $\{\mb{s}(t)\} = \{\mb{u}(t), \partial_{t} \mb{u}(t)\}$. In particular, it needs a nontrivial starting procedure that evaluates $\partial_{t} \mb{u}(t_{0})$ by enforcing a zero residual at $t_{0}$. The finaliser can still return the first vector of the state vectors. For convenience, let $\partial_{t} \mb{u}_{n}$ denote the approximation $\partial_{t} \mb{u}(t_{n})$.

This method extends the $\theta$-method by considering the two-point quadrature rule $$\mb{u}(t_{n+1}) = \mb{u}_{n} + \int_{t_{n}}^{t_{n+1}} \partial_{t} \mb{u}(t) \D{t} \approx \mb{u}_{n} + h_{n} [(1 - \gamma) \partial_{t} \mb{u}(t_{n}) + \gamma \partial_{t} \mb{u}(t_{n+1})],$$
where $0 \leq \gamma \leq 1$ is a free parameter. The question is now how to estimate $\partial_{t} \mb{u}(t_{n+1})$. This is achieved by enforcing a zero residual at $t_{n + \alpha_{F}} \doteq (1 - \alpha_{F}) t_{n} + \alpha_{F} t_{n+1}$, where $0 \leq \alpha_{F} \leq 1$ is another free parameter. The value of $\mb{u}$ at that time, $\mb{u}_{n + \alpha_{F}}$, is obtained by the same linear combination of $\mb{u}$ at $t_{n}$ and $t_{n+1}$. Regarding $\partial_{t} \mb{u}$, it is taken as a linear combination weighted by another free parameter $0 < \alpha_{M} \leq 1$ of the time derivative at times $t_{n}$ and $t_{n+1}$. Note that this time $\alpha_{M}$ cannot be zero. Altogether, we have defined the discrete operators
$$\begin{align*}
t_{1} &= (1 - \alpha_{F}) t_{n} +  \alpha_{F} t_{n+1}, \\
\Delta_{1}^{0} &= (1 - \alpha_{F}) \mb{u}_{n} + \alpha_{F} \mb{u}_{n+1}, \\
\Delta_{1}^{1} &= (1 - \alpha_{M}) \partial_{t} \mb{u}_{n} + \alpha_{M} \partial_{t} \mb{u}_{n+1}.
\end{align*}$$

In more concrete terms, we solve the following system:
$$\begin{align*}
\mb{0} &= \mb{r}(t_{n + \alpha_{F}}, \mb{u}_{n + \alpha_{F}}, \partial_{t} \mb{u}_{n + \alpha_{M}}), \\
t_{n + \alpha_{F}} &= (1 - \alpha_{F}) t_{n} + \alpha_{F} t_{n+1}, \\
\mb{u}_{n + \alpha_{F}} &= (1 - \alpha_{F}) \mb{u}_{n} + \alpha_{F} \mb{u}_{n+1}, \\
\partial_{t} \mb{u}_{n + \alpha_{M}} &= (1 - \alpha_{M}) \partial_{t} \mb{u}_{n} + \alpha_{M} \partial_{t} \mb{u}_{n+1}, \\
\mb{u}_{n+1} &= \mb{u}_{n} + h_{n} [(1 - \gamma) \partial_{t} \mb{u}_{n} + \gamma \mb{x}], \\
\partial_{t} \mb{u}_{n+1} &= \mb{x}.
\end{align*}$$
The state vector is updated to $\{\mb{s}\}_{n+1} = \{\mb{u}_{n+1}, \partial_{t} \mb{u}_{n+1}\}$.

**Analysis**
The amplification matrix for the state vector is
$$\mb{A}(z) = \frac{1}{\alpha_{M} - \alpha_{F} \gamma z} \begin{bmatrix}\alpha_{M} + (1 - \alpha_{F}) \gamma z & \alpha_{M} - \gamma \\ z & \alpha_{M} - 1 + \alpha_{F} (1 - \gamma) z\end{bmatrix}.$$
It is then immediate to see that $\mb{u}_{n+1} = \tr(\mb{A}) \mb{u}_{n} - \det(\mb{A}) \mb{u}_{n-1}$. This time, plugging the Taylor expansion of $\mb{u}_{n+1}$ and $\mb{u}_{n-1}$ around $\mb{u}_{n}$ in this expression, the exactness condition is $\tr(\mb{A}(z)) - \det(\mb{A}(z)) \exp(-z) - \exp(z) = 0$. To simplify the analysis, we write the trace and determinant of $\mb{A}$ as follows
$$\tr(\mb{A}(z)) = a + \frac{b}{1 - c z}, \qquad \det(\mb{A}(z)) = d + \frac{e}{1 - c z},$$
where
$$\begin{align*}
...
The order conditions are as follows.
* **Order 0 and 1**. The first two coefficients are always zero, so the method has always order $2$.
...
We finally study the stability in the extreme cases $\abs{z} \to 0$ and $\abs{z} \to +\infty$. We want the spectral radius of the amplification matrix to be smaller than one, so that perturbations are damped away.
* When $\abs{z} \to 0$, we have $\rho(\mb{A}(z)) \to \max\{1, \abs{1 - \frac{1}{\alpha_{M}}}\}$.
* When $\abs{z} \to +\infty$, we have $\rho(\mb{A}(z)) \to \max\{\abs{1 - \frac{1}{\alpha_{F}}} and \abs{1 - \frac{1}{\gamma}}\}$.
...
This scheme was originally devised to control the damping of high frequencies. One parameteristation consists in prescribing the eigen values at $\abs{z} \to +\infty$, and this leads to
...
## Runge-Kutta
Runge-Kutta methods are multi-stage, i.e. they build estimates of $\mb{u}$ at intermediate times between $t_{n}$ and $t_{n+1}$. They can be written as follows
$$\begin{align*}
\mb{0} &= \mb{r}(t_{n} + c_{i} h_{n} , \mb{u}_{n} + \sum_{1 \leq j \leq s} a_{ij} h_{n} \mb{x}_{j}, \mb{x}_{i}), & 1 \leq i \leq p \\
\mb{u}_{n+1} &= \mb{u}_{n} + \sum_{1 \leq i \leq p} b_{i} h_{n} \mb{x}_{i},
\end{align*}$$
where $p$ is the number of stages, $\mb{A} = (a_{ij})_{1 \leq i, j \leq p}$ is a matrix of free parameters, $\mb{b} = (b_{i})_{1 \leq i \leq p}$ and $\mb{c} = (c_{i})_{1 \leq i \leq p}$ are two vectors of free parameters. The stage unknowns $(\mb{x}_{i})_{1 \leq i \leq p}$ are involved in a coupled system of equations. This system can take a simpler form when the matrix $\mb{A}$ has a particular structure.
* When $\mb{A}$ is lower triangular, the equations are decoupled and can thus be solved sequentially. These schemes are called Diagonally-Implicit Runge-Kutta (DIRK). If the diagonal coefficients of the matrix $\mb{A}$ are the same, the method is called Singly-Diagonally Implicit (SDIRK).
* If additionally the diagonal coefficients are zero, the method is explicit. These schemes are called Explicit Runge-Kutta (EXRK).

**Implementation details** It is particularly advantageous to save the factorisation of the matrices of the stage operators for Runge-Kutta methods. This is always possible when the method is explicit and the mass matrix is constant, in which case all the stage matrices are the mass matrix. When the method is diagonally-implicit and the stiffness and mass matrices are constant, the matrices of the stage operators are $\mb{M} + a_{ii} h_{n} \mb{K}$. In particular, if two diagonal coefficients coincide, the corresponding operators will have the same matrix. We implement these reuse strategies by storing them in `CompressedArray`s, and introducing a map `i -> NumericalSetup`.

**Analysis**
The stability function of a Runge-Kutta scheme is
$$\rho(z) = 1 + z \mb{b}^{T} (\mb{I} - z \mb{A})^{-1} \mb{1}.$$
...
When the residual has an implicit-explicit decomposition, usually because we can identify a stiff part that we want to solve implicit and a nonstiff part that we want to solve explicitly, the Runge-Kutta method reads as follows
$$\begin{align*}
\mb{0} &= \mb{r}(t_{n} + c_{i} h_{n}, \mb{u}_{n} + \sum_{1 \leq j \leq i-1} (a_{i, j} h_{n} \mb{x}_{j} + \hat{a}_{i, j} h_{n} \hat{\mb{x}}_{j}) + a_{i, i} h_{n} \mb{x}_{i}, \mb{x}_{i}), \\
\mb{0} &= \hat{\mb{r}}(t_{n} + c_{i} h_{n}, \mb{u}_{n} + \sum_{1 \leq j \leq i-1} (a_{i, j} h_{n} \mb{x}_{j} + \hat{a}_{i, j} h_{n} \hat{\mb{x}}_{j}) + a_{i, i} h_{n} \mb{x}_{i}, \hat{\mb{x}}_{i}), & 1 \leq i \leq p \\
\mb{u}_{n+1} &= \mb{u}_{n} + \sum_{1 \leq i \leq p} (b_{i} h_{n} \mb{x}_{i} + \hat{b}_{i} h_{n} \hat{\mb{x}}_{i}).
\end{align*}$$
In these expressions, quantities that wear a hat are the explicit counterparts of the implicit quantity with the same name. The implicit and explicit stages are alternated, i.e. the implicit and explicit stage unknowns $\mb{x}_{i}$ and $\hat{\mb{x}}_{i}$ are solved alternatively. As can be seen above, we require that the nodes $c_{i}$ of the implicit and explicit tableaus coincide. This implies that the first step for the implicit part is actually explicit.
...
This scheme relies on the state vector $\{\mb{s}(t)\} = \{\mb{u}(t), \partial_{t} \mb{u}(t), \partial_{tt} \mb{u}(t)\}$. It needs a nontrivial starting procedure that evaluates $\partial_{tt} \mb{u}(t_{0})$ by enforcing a zero residual at $t_{0}$. The finaliser can still return the first vector of the state vectors. For convenience, let $\partial_{tt} \mb{u}_{n}$ denote the approximation $\partial_{tt} \mb{u}(t_{n})$.

This methods is built out of the following update rule
$$\begin{align*}
\mb{0} &= \mb{r}(t_{n + 1 - \alpha_{F}}, \mb{u}_{n + 1 - \alpha_{F}}, \partial_{t} \mb{u}_{n + 1 - \alpha_{F}}, \partial_{tt} \mb{u}_{n + 1 - \alpha_{M}}), \\
t_{n + 1 - \alpha_{F}} &= \alpha_{F} t_{n} + (1 - \alpha_{F}) t_{n+1}, \\
\mb{u}_{n + 1 - \alpha_{F}} &= \alpha_{F} \mb{u}_{n} + (1 - \alpha_{F}) \mb{u}_{n+1}, \\
\partial_{t} \mb{u}_{n + 1 - \alpha_{F}} &= \alpha_{F} \partial_{t} \mb{u}_{n} + (1 - \alpha_{F}) \partial_{t} \mb{u}_{n+1}, \\
\partial_{tt} \mb{u}_{n + 1 - \alpha_{M}} &= \alpha_{M} \partial_{tt} \mb{u}_{n} + (1 - \alpha_{M}) \partial_{tt} \mb{u}_{n+1}, \\
\mb{u}_{n+1} &= \mb{u}_{n} + h_{n} \partial_{t} \mb{u}_{n} + \frac{1}{2} h_{n}^{2} [(1 - 2 \beta) \partial_{tt} \mb{u}_{n} + 2 \beta \mb{x}] \\
\partial_{t} \mb{u}_{n+1} &= \partial_{t} \mb{u}_{n} + h_{n} [(1 - \gamma) \partial_{tt} \mb{u}_{n} + \gamma \mb{x}], \\
\partial_{tt} \mb{u}_{n+1} &= \mb{x}
\end{align*}$$
The state vector is then updated to $\{\mb{s}\}_{n+1} = \{\mb{u}_{n+1}, \partial_{t} \mb{u}_{n+1}, \partial_{tt} \mb{u}_{n+1}\}$.

**Analysis** The amplification matrix for the state vector is
$$\mb{A}(z) = \frac{1}{1 - \alpha_{M} + (1 - \alpha_{F}) \beta z^{2}} \begin{bmatrix}
...
\end{bmatrix}.$$
Here again, we immediately see that $\mb{u}_{n+1}$ satisfies the recurrence
$$\mb{u}_{n+1} = \tr(\mb{A}(z)) \mb{u}_{n} - \frac{1}{2} (\tr(\mb{A}(z))^{2} - \tr(\mb{A}(z)^{2})) \mb{u}_{n-1} + \det(\mb{A}(z)) \mb{u}_{n-2}.$$
By plugging the Taylor expansion of $\mb{u}$ at times $t_{n+1}$, $t_{n-1}$ and $t_{n-2}$, we obtain the exactness condition
$$\cos(z) = \tr(\mb{A}(z)) - \frac{1}{2} (\tr(\mb{A}(z))^{2} - \tr(\mb{A}(z)^{2})) \cos(z) + \det(\mb{A}(z)) \cos(2z).$$
These conditions are hard to examine analytically, but one can verify that this scheme is always of order one. Second-order is achieved by setting $\gamma = \frac{1}{2} - \alpha_{M} + \alpha_{F}$.

It is easier to consider the limit cases $\abs{z} \to 0$ and $\abs{z} \to +\infty$ and look at the eigen values of the amplification matrix.
* When $\mb{z} \to 0$, we find $\rho(\mb{A}(z)) = \max\{1, \abs{\frac{\alpha_{M}}{1 - \alpha_{M}}}\}$.
* When $\mb{z} \to +\infty$, we find $\rho(\mb{A}(z)) = \max\{\abs{\frac{\alpha_{F}}{1 - \alpha_{F}}}, \abs{\frac{4 \beta - (1 + 2 \gamma) \pm \sqrt{(1 + 2 \gamma)^{2} - 16 \beta}}{4 \beta}}\}$.

For all these eigen values to have a modulus smaller than one, we need $\alpha_{M} \leq \frac{1}{2}$, $\alpha_{F} \leq \frac{1}{2}$, $\gamma \geq \frac{1}{2}$, i.e. $\alpha_{F} \geq \alpha_{M}$ and $\beta \geq \frac{1}{2} \gamma$. Since dissipation of high-frequency is maximised when the eigen values are real at infinity, we also impose $\beta = \frac{1}{16} (1 + 2 \gamma)^{2}$, i.e. $\beta = \frac{1}{4} (1 - \alpha_{M} + \alpha_{F})^{2}$.
...
# TODO
Some Runge-Kutta schemes have the First-Same-As-Last (FSAL) property, that enables to share a residual evaluation from one step to the next. It may not be possible to have this optimisation in our case because of the Dirichlet boundary conditions.
